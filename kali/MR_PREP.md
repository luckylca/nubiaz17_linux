...
- `Docker`: true-device runtime E2E is PASS (`run/exec/mqueue/bind/cgroup/bridge/private-netns port publish/cleanup`). The isolated Docker netns is attached to the Android host netns through `slirp4netns`; container→Android-host traffic is proven and Android global Docker-related iptables rules remain unchanged. On 2026-09-21 the slirp API `add_hostfwd` path also passed Android-loopback and Mac→ADB-forward client tests, so reverse host-port publishing is proven without opening a LAN listener. The phone still had `mDefaultNetwork=null`, so real container Internet remains the only missing online datapoint. Keep the official `Docker` feature out of the first MR until that is validated.
