...
| Docker Engine runtime | 2026-09-20: Docker-test kernel + Docker 29.1.3/containerd 1.7.35 真机 E2E；离线 ARM64 image import、`docker run` + `/dev/mqueue`、`docker exec`、bind mount、memory/cpu-shares cgroup、bridge HTTP、私有 netns 内 `-p 18080:8080`、daemon info、退出后 host mount leak 全部 PASS；随后加入 slirp4netns，容器 `172.17.0.2` → docker0/NAT → `tap0 10.0.2.100` → Android host HTTP 数据面实收 | **PASS(runtime + user-mode uplink datapath)** | Android `/data` 上 downstream 4.4 overlayfs 不支持 upperdir，因此用 `vfs`；Docker 运行在独立 mount+network ns，slirp4netns 提供不修改 Android 全局 firewall 的用户态 uplink，E2E 同时得到 `ANDROID_DOCKER_FIREWALL_UNCHANGED_PASS`。测试时宿主无任何已保存 Wi-Fi/外网路由，因此真实容器 Internet 仍待在线复测；首版官方 `Docker` feature 暂不声明 |
