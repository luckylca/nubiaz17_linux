...
| Docker Engine runtime | 2026-09-21: Docker-test kernel + Docker 29.1.3/containerd 1.7.35 真机 E2E；离线 ARM64 image import、`docker run` + `/dev/mqueue`、`docker exec`、bind mount、memory/cpu-shares cgroup、bridge HTTP、私有 netns 内 `-p 18080:8080`、daemon info、退出后 host mount leak 全部 PASS；slirp4netns 提供 `tap0 10.0.2.100` 用户态 uplink，容器→Android host HTTP PASS；进一步用 slirp API `add_hostfwd` 将 Android `127.0.0.1:18082` 映射到私有 netns `10.0.2.100:18080`，Android loopback 与 Mac→ADB-forward 两条客户端路径均实收 `NX563J_HTTP_OK` | **PASS(runtime + user-mode uplink + hostfwd datapath)** | Android `/data` 上 downstream 4.4 overlayfs 不支持 upperdir，因此用 `vfs`；Docker 运行在独立 mount+network ns，slirp4netns 不修改 Android 全局 firewall，E2E 得到 `ANDROID_DOCKER_FIREWALL_UNCHANGED_PASS`。宿主当前 `mDefaultNetwork=null` 且 Android `curl` 返回 HTTP `000`，因此真实容器 Internet 仍待宿主联网后复测；首版官方 `Docker` feature 暂不声明 |
